...
const connectActions = {
  ...
  async resetCartToken ({ getters, dispatch, commit }, { guestCart = false, forceClientState = false, mergeQty = false }) {
    if (!getters.isCartSyncEnabled) return
    const { result, resultCode } = await CartService.getCartToken(guestCart, forceClientState)
    ...
    if (resultCode === 200) {
      ...
      dispatch('sync', { forceClientState, dryRun: !config.cart.serverMergeByDefault, mergeQty })
      return result
    }
